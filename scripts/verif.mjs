...
/* 7 bis. Les mentions : tant que src/data/affiliation.json dit « actif: false », aucune page ne
   dit qu'on touche une commission (décision de Jordane du 23/09/2026). La greffe les retire des
   pages portées ; ce contrôle relit le texte visible pour qu'une maquette ou un composant qui en
   ajouterait une ne passe pas. */
const MENTIONS_PERMISES = /Le site ne porte aucun identifiant d'affiliation/g;
function verifMentionsAffiliation() {
  if (AFFILIATION.actif) return [];
  const trouvees = [];
  for (const f of pages) {
    const texte = lu(f).replace(/<(script|style)[^>]*>[\s\S]*?<\/\1>/g, ' ').replace(/<[^>]+>/g, ' ').replace(MENTIONS_PERMISES, '');
    const m = texte.match(/[^.]{0,50}(?:affili|commission)[^.]{0,50}/i);
    if (m) trouvees.push({ page: route(f), extrait: m[0].replace(/\s+/g, ' ').trim() });
  }
  return trouvees;
}
...
/* 9. Balisage — une balise mal fermée, une grille refermée trop tôt, un guillemet non échappé.
   Le jugement est dans `scripts/balisage.mjs`, isolé pour que `tests/balisage.test.mjs` puisse
   le passer sur des cas connus : un contrôle qu'on ne teste pas est une promesse, pas un filet.
   Ici ne restent que le parcours des pages et le rendu. */

const balisage = pages
  .map((f) => ({ page: route(f), defauts: defautsDeBalisage(lu(f)) }))
